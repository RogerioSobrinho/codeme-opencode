...
echo "  Rules    : $TOTAL_RULES  (modular instruction sets)"
  echo "  Plugins  : $TOTAL_PLUGINS  (typescript-check, lint-check, pre-commit-guard, bash-guard, session-notify, env-protection, flutter-check, java-check, shell-env, compaction, todo-progress, session-timer, diff-summary, file-backup, auto-branch, session-summary, stale-todo-guard, error-rerun, smart-context, daily-digest, session-error-notify, tui-toast, lsp-diagnostics, permission-auto-approve, command-history, doom-loop-notify, file-watcher, session-diff, tui-prompt-shortcuts, context-window-monitor, directory-agents-injector, directory-readme-injector, preemptive-compaction, tool-output-truncator, write-guard)"
...
echo ""
  ...
  echo "       /clean-slop  /commit  /review-work  /handoff"
  echo ""
echo ""
